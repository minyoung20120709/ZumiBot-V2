...
#######################################################################################################################
    ...
    async def cmd_summon(self, channel, guild, author, voice_channel):
        ...
        if not author.voice:
            raise exceptions.CommandError(self.str.get('cmd-summon-novc', '음성 채널에 연결되지 않았습니다. 음성채널에 참가한 후 다시 시도하십시오.'))
        ...
        if voice_client and guild == author.voice.channel.guild:
            ...
            if not chperms.connect:
                ...
                log.warning("권한이 없습니다.'{0}'".format(author.voice.channel.name))
                raise exceptions.CommandError(
